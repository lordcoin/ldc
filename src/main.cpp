...
#if defined(NDEBUG)
# error "Lordcoin cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 7 * 60 * 60; // 8 hours
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Lordcoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Lordcoin: find block wanted by given orphan block
...
// Lordcoin: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Lordcoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Lordcoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Lordcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Lordcoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Lordcoin: track money supply and mint amount info
...
// Lordcoin: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Lordcoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Lordcoin: compute stake entropy bit for stake modifier
    ...
    // Lordcoin: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Lordcoin: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Lordcoin: attempt to generate suitable proof-of-stake
